...
symmetrize <- function(mat) {
  # Before proceeding, must check that mat meets requirements.
  if (!any(c("matrix", "Matrix", 'dgeMatrix') %in% class(mat))) {
    stop("Input for symmetrize must be of class 'matrix'.
                Inputted object is of class ", class(mat))
  } else if (nrow(mat) < ncol(mat)){
    stop("Input for symmetrize must be a matrix with at least as many rows as columns. 
                Dimensions of inputted matrix are: (", nrow(mat), ", ", ncol(mat), ").")
  }
  # Determine number of columns, which we will use for indexing.
  N <- ncol(mat)
  # First, zero out negative entries.
  mat.symmetric <- abs(mat)/2 + mat/2
  # Next, isolate the top submatrix.
  mat.submatrix <- mat[1:N, ]
  # Take average of the off-diagonal pairs in submatrix
  # in order to balance out directions.
  mat.submatrix <- mat.submatrix/2 + t(mat.submatrix)/2
  # Replace diagonals of submatrix with zeros.
  diag(mat.submatrix) <- 0
  # Insert submatrix into larger matrix to complete symmetrization.
  mat.symmetric[1:N, ] <- mat.submatrix 
